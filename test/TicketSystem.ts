...
describe("TicketSystem Testing", () => {
    ...
    describe('minting test', () => {
        ...
        it('token metadata', async () => {
            ...
            const stringifiedMetadata = await Buffer.from(metadata, "base64").toString('ascii');
    ...
    describe("client adding verify", () => {
        it('adding client', async () => {
            ...
            const {deployed} = await setup({});
            await deployed.createClient(client.fullname, client.fisicalAddress, client.telefone);
            let a = await deployed.clientData(0)
            expect(a.toString()).to.equal([client.fullname, client.fisicalAddress, client.telefone].toString())
    ...
    describe("redeem testing", () => {
        ...
        it('token burned', async () => {
            ...
            await expect(deployed.ownerOf(0)).to.be.revertedWith("ERC721: invalid token ID")
        })
        it('client received', async () => {
            ...
            expect(b.toString()).to.equal([client.fullname, client.fisicalAddress, client.telefone].toString())
        })
    })
